...
一个用 C++ 实现的可配置 Cache 替换策略模拟器，是 **2020 年清华大学（THU）计算机体系结构课程作业**。程序读取内存访问 trace，在指定 Cache 容量、块大小、相联度、替换策略和写分配策略后，统计命中数、缺失数和缺失率。

## 项目背景

本项目完成于 2020 年清华大学计算机体系结构课程，用于理解 Cache 的地址映射、组相联结构和替换策略。实现重点是将 Cache 数据与替换策略分离：Cache 负责地址解析、tag 比较及读写流程，各替换算法通过统一接口独立实现。
